"""慕鸢の公益站 (newapi.linuxdo.edu.rs) - 自动签到"""
import json
import os
import re
import sys
...
def fmt_quota(q):
    if q is None:
        return "未知"
    return f"${q / 500_000:.2f}"


def build_msg(uname, result, quota_info=None):
    lines = ["🕊️ 慕鸢签到", f"用户: {uname}"]
...
def run():
    ...
    proxy = get_proxy()
    client = MuYuanClient(proxy=proxy)

    saved = load_session()
    if saved and saved.get("token"):
        client.session.cookies.set("session", saved["token"], domain="newapi.linuxdo.edu.rs")
